...
class IQuestionDistributionResolver[QuestionT](Protocol):
    def at(self, difficulty: int) -> QuestionDistribution[QuestionT]: ...


@frozen
class DifficultyQuestionDistribution[QuestionT](IQuestionDistributionResolver[QuestionT]):
...
class IRegisteredQuestion(Protocol):
    @property
    def id(self) -> str: ...


class QuestionCatalog[QuestionT: IRegisteredQuestion](Mapping[str, QuestionT]):
